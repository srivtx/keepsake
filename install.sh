#!/bin/sh
# install.sh — install keepsake from GitHub.
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/keepsake/main/install.sh | sh
#
# Installs the `keepsake` binary globally with Bun. Pass a git ref as the
# first argument to pin a branch, tag, or commit:
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/keepsake/main/install.sh | sh -s v0.1.0
set -eu

REPO="srivtx/keepsake"
BIN="keepsake"
REF="${1:-main}"
